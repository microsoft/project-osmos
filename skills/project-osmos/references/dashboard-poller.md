...
On re-engage, read `terminal.json` first, then `state.json`. If the user is only asking for status and `terminal.json` exists, report its `reason` (including `signal`, `max_runtime`, and `no_progress_window`) before offering to resume. A user-authored follow-up is already authorization to continue: follow [Task lifecycle → Continue a task](task-lifecycle.md#continue-a-task), and respawn the poller without an additional confirmation only when the helper reports `poller_restart_required: true`. If `terminal.json` is absent and the recorded PID is not alive, treat it as an ungraceful death (machine sleep, reboot, OOM, `kill -9`) and respawn.
